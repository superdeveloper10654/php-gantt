	...
	function _render_grid_item(item, view) {
		...
		for (var i = 0; i < columns.length; i++) {
			...
			if (col.name == "add") {
				...
				value = "<div " + aria + " class='gantt_add' title='Insert a new Task'</div>";
				textValue = "";
			} else {
				if (col.template)
...
function createResourceMethods(gantt){
	...
	function getResourceTasks(property, resourceIds) {
		...
		var resourceToSearchOn = resourceIds[0];
		...
		if(!resourceTaskCache[cacheKey]){
			...
			gantt.eachTask(function (task) {
				...
				var taskResArray = task.resource_id;
				if (task.resource_id != null)
			{
				if (taskResArray.indexOf(',') > -1) 
				{ 
	
				 var arrayParsed = taskResArray.split(',');
				 for(var i = 0; i < arrayParsed.length; i++)
				{
							if (resourceToSearchOn == arrayParsed[i])
								{
				...
				else 
				{
					...
					if (task.resource_id == resourceToSearchOn)
					{
						res.push(task);
					}
				}
