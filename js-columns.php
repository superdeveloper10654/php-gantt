    ...
    if (columnName == "status") {
      object = {
        ...
        template: function(task) {
          ...
          if (task.progress == "100" && task.manually_delayed == "0" && task.actual_cost_completion > task.budget_at_completion) {
          ...
          if (task.manually_delayed == "1") {
            ...
            if (moment(task.start_date).isAfter(moment(task.baseline_start)) && moment(task.end_date).isAfter(moment(task.baseline_end)) && task.progress == "0") {
              return "<img src='img/svg/delayed.svg' title='Delayed (not started yet)'>";
            } 
           if (moment(task.start_date).isAfter(moment(task.baseline_start)) && moment().isAfter(moment(task.start_date)) && moment(task.end_date).isAfter(moment(task.baseline_end)) && task.progress != "0") {
              return "<img src='img/svg/delayed.svg' title='Delayed (in progress)'>";
            } 
            if (moment(task.start_date).isBefore(moment(task.baseline_start)) && moment(task.end_date).isBefore(moment(task.baseline_end)) && task.progress == "0") {
             // return "<img src='img/svg/accelerated.svg' title='Accelerated (not started yet)'>";
            } 
           if (moment(task.start_date).isBefore(moment(task.baseline_start)) && moment().isAfter(moment(task.start_date)) && moment(task.end_date).isBefore(moment(task.baseline_end)) && task.progress != "0") {
              return "<img src='img/svg/accelerated.svg' title='Accelerated (in progress)'>";
            } 
          if (moment(task.start_date).isSame(moment(task.baseline_start)) && moment(task.end_date).isAfter(moment(task.baseline_end)) && task.progress != "0") {
            return "<img src='img/svg/prolonged.svg' title='Prolonged'>";
          }
if (moment(task.start_date).isSame(moment(task.baseline_start)) && moment(task.end_date).isAfter(moment(task.baseline_end)) && task.progress == "0") {
            return "<img src='img/svg/prolonged.svg' title='Prolonged (not started yet)'>";
          }
          ...
          if (task.manually_delayed == "0" && task.progress == "0") {
            return "<img src='img/svg/pending.svg' title='Not started yet'>";
          }
          if (moment(task.start_date).isSame(moment(task.baseline_start)) && moment(task.start_date).isBefore(moment()) && moment(task.baseline_end).isSame(moment(task.end_date)) && task.manually_delayed == "0" && task.progress != "0" && task.progress != "100") {
            return "<img src='img/svg/in-progress.svg' title='In Progress (on schedule)'>";
          }
      ...
      object = {
        ...
        template: function(task) {
          if (window.ibex_gantt_config.periodDescriptor == "1") {
            ...
            if (window.ibex_gantt_config.periodDescriptor == "5") { // Sessions for example
